...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        """MERGED: Combines dual currency (balance_ref) with branch column.
        
        - l10n_ve_account_reports_dual: Uses balance_ref, amount_ref for ref currency
        - branch_accounting_report: Adds branch.name column and LEFT JOIN res_branch
        """
        ...
        # Dual currency support - choose balance fields based on selected currency
        use_ref_currency = options.get('selected_currency') == self.env.company.currency_ref_id.id

        if use_ref_currency:
            # Use reference currency fields (from l10n_ve_account_reports_dual)
            select_period_query = ','.join(
                f"""
                    CASE WHEN period_table.period_index = {i}
                    THEN %s * (
                        SUM(account_move_line.balance_ref)
                        - COALESCE(SUM(part_debit.amount_ref), 0)
                        + COALESCE(SUM(part_credit.amount_ref), 0)
                    )
                    ELSE 0 END AS period{i}
                """
                for i in range(len(periods))
            )
            part_amount_field = 'amount_ref'
            balance_field = 'balance_ref'
        else:
            # Use standard fields with currency table rate (from branch_accounting_report)
            select_period_query = ','.join(
                f"""
                    CASE WHEN period_table.period_index = {i}
                    THEN %s * (
        ...
        query = f"""
            ...
            LEFT JOIN LATERAL (
                SELECT SUM(part.{part_amount_field}) AS {part_amount_field}, part.debit_move_id
            ...
            LEFT JOIN LATERAL (
                SELECT SUM(part.{part_amount_field}) AS {part_amount_field}, part.credit_move_id
                FROM account_partial_reconcile part
                WHERE part.max_date <= %s
            ...
            HAVING (
                SUM(ROUND(account_move_line.{balance_field} * currency_table.rate, currency_table.precision))
                - COALESCE(SUM(ROUND(part_debit.{part_amount_field} * currency_table.rate, currency_table.precision)), 0)
                + COALESCE(SUM(ROUND(part_credit.{part_amount_field} * currency_table.rate, currency_table.precision)), 0)
            ) != 0
            {tail_query}
        """
