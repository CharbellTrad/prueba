...
class WithholdingIVATXT(models.Model):
	...
	def _generate_txt_data(self):
		...
		for withholding_id in self.line_ids:
			if not withholding_id.subject_id.vat:
				if withholding_id.subject_id.name not in partners_without_rif:
					partners_without_rif.append(withholding_id.subject_id.name)
			
			if not withholding_id.agent_id.vat:
				if withholding_id.agent_id.name not in partners_without_rif:
					partners_without_rif.append(withholding_id.agent_id.name)
		...
		for withholding_id in self.line_ids:
			...
			agent_rif = (withholding_id.agent_id.vat or '').replace('-', '')
			...
			for line in withholding_id.line_ids:
				data.append('\t'.join((
					...
					invoice_date,
					'C',
					invoice.fiscal_transaction_type[-2:] if invoice.fiscal_transaction_type else '',
					subject_rif,
