...
class AccountWithholdingIVA(models.Model):
	...
	def button_pay_withholding(self):
		""" Crea un pago de proveedor al SENIAT por el monto retenido.
		...
		# Buscar el diario de banco de la compañía
		journal = self.env['account.journal'].search(
			[('type', 'in', ('bank', 'cash')), ('company_id', '=', self.company_id.id), ('currency_id', 'in', (False, self.company_id.currency_id.id))],
			limit=1
		)
		if not journal:
			raise UserError('No se encontró un diario de banco o efectivo para registrar el pago.')
		
		# La cuenta destino del pago es la cuenta donde entra en débito el asiento
		# de la retención, es decir, la withholding_account_id (cuenta de retención).
		# Esto sustituye la cuenta por pagar nativa del contacto SENIAT.
		...
		if rate_ref_id:
			payment_vals['currency_rate_ref'] = rate_ref_id
				
		payment = self.env['account.payment'].create(payment_vals)
		self.payment_id = payment
		return {
			'name': 'Pago de Retención',
			'type': 'ir.actions.act_window',
			'res_model': 'account.payment',
			'view_mode': 'form',
			'res_id': payment.id,
