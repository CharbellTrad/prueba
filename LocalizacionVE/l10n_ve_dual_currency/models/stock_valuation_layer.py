...
class StockMove(models.Model):
	...
	def _get_in_svl_vals(self, forced_quantity):
		"""Override: inyecta la tasa del picking en el contexto para que
		_prepare_in_svl_vals use la tasa correcta en value_ref."""
		svl_vals_list = []
		for move in self:
			...
			# Inyectar tasa del picking/OC en contexto
			rate = False
			if move.currency_rate_ref and move.currency_rate_ref.rate:
				rate = move.currency_rate_ref.rate
			product = move.product_id.with_context(stock_move_rate_ref=rate) if rate else move.product_id
			svl_vals = product._prepare_in_svl_vals(forced_quantity or valued_quantity, unit_cost)
	...
	def product_price_update_before_done(self, forced_qty=None):
		...
		for move in self.filtered(lambda move: move._is_in() and move.with_company(move.company_id).product_id.cost_method == 'average'):
			...
			# Usar la tasa del picking/OC si está disponible
			rate = move.currency_rate_ref.rate if move.currency_rate_ref and move.currency_rate_ref.rate else move.company_id.currency_ref_id.rate

			# Write the standard price, as SUPERUSER_ID because a warehouse manager may not have the right to write on products
			move.product_id.with_company(move.company_id.id).with_context(disable_auto_svl=True).sudo().write({
				'standard_price': new_std_price,
				'standard_price_ref': new_std_price * rate,
			})
