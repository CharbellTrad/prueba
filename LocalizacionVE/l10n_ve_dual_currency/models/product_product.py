...
class ProductProduct(models.Model):
	...
	def write(self, vals):
		if 'standard_price' in vals and 'standard_price_ref' not in vals:
			vals['standard_price_ref'] = self.cost_currency_ref_id.round(vals['standard_price'] * self.cost_currency_ref_id.rate)
		elif 'standard_price_ref' in vals and 'standard_price' not in vals:
			vals['standard_price'] = self.cost_currency_id.round(vals['standard_price_ref'] / self.cost_currency_ref_id.rate)
	...
	def _prepare_in_svl_vals(self, quantity, unit_cost):
		...
		# Usar la tasa del stock.move (proveniente del picking/OC) si está en el contexto
		rate = self.env.context.get('stock_move_rate_ref', False)
		if not rate:
			rate = company.currency_ref_id.rate
		value_ref = company.currency_ref_id.round(unit_cost * quantity * rate)
		vals.update({
			'value_ref': value_ref,
			'unit_cost_ref': company.currency_ref_id.round(unit_cost * rate),
