...
class AccountMoveLine(models.Model):
	...
	def _compute_currency_rate_ref(self):
		for line in self:
			if self.env.context.get('pos_session_no_header_rate'):
				line.currency_rate_ref = line.currency_rate_ref or line.currency_ref_id.get_currency_rate()
			else:
	...
	def _prepare_reconciliation_single_partial(self, debit_vals, credit_vals):
		...
		if debit_vals['currency'] == credit_vals['currency'] == company_currency \
			...
			recon_credit_amount = -remaining_credit_amount
				...
				and credit_vals['currency'] != currency_ref_id \
			...
			if recon_currency.is_zero(recon_debit_amount) or recon_currency.is_zero(recon_credit_amount):
				...
				and debit_vals['currency'] != currency_ref_id \
				and is_rec_pay_account \
		...
		debit_rate_ref = abs(remaining_debit_amount_curr_ref / remaining_debit_amount) if remaining_debit_amount else 0.0
		credit_rate_ref = abs(remaining_credit_amount_curr_ref / remaining_credit_amount) if remaining_credit_amount else 0.0
		...
		if not self._context.get('no_exchange_difference'):
			...
			def _add_exchange_amount(record, key, amount):
				nonlocal exchange_lines_to_fix
				if not record:
					return
				currency_to_round = record.currency_id if key == 'amount_residual_currency' else (
					record.company_id.currency_ref_id if key == 'amount_residual_currency_ref' else record.company_currency_id)
				amount_rounded = currency_to_round.round(amount)
				if currency_to_round.is_zero(amount_rounded):
					return
				
				if record not in exchange_lines_to_fix:
					exchange_lines_to_fix += record
					amounts_by_record[record.id] = {}
				amounts_by_record[record.id][key] = amounts_by_record[record.id].get(key, 0.0) + amount_rounded

			if recon_currency == company_currency:
				if debit_fully_matched:
					debit_exchange_amount = remaining_debit_amount_curr - partial_debit_amount_currency
					_add_exchange_amount(debit_vals.get('record'), 'amount_residual_currency', debit_exchange_amount)
				...
				if credit_fully_matched:
					...
					_add_exchange_amount(credit_vals.get('record'), 'amount_residual_currency', credit_exchange_amount)
					remaining_credit_amount_curr += credit_exchange_amount

			else:
				if debit_fully_matched:
					debit_exchange_amount = remaining_debit_amount - partial_amount
					_add_exchange_amount(debit_vals.get('record'), 'amount_residual', debit_exchange_amount)
				...
				if credit_fully_matched:
					...
					_add_exchange_amount(credit_vals.get('record'), 'amount_residual', credit_exchange_amount)
			...
			if debit_fully_matched:
				...
				_add_exchange_amount(debit_vals.get('record'), 'amount_residual_currency_ref', debit_exchange_amount_ref)
				remaining_debit_amount_curr_ref -= debit_exchange_amount_ref
			else:
			...
			if credit_fully_matched:
				...
				_add_exchange_amount(credit_vals.get('record'), 'amount_residual_currency_ref', credit_exchange_amount_ref)
				remaining_credit_amount_curr_ref -= credit_exchange_amount_ref
			else:
			...
			if exchange_lines_to_fix:
				# Convert the dictionary into a list that perfectly maintains index parity with exchange_lines_to_fix
				amounts_list = [amounts_by_record[record.id] for record in exchange_lines_to_fix]
		...
		debit_vals['amount_residual_ref'] = remaining_debit_amount_curr_ref
		credit_vals['amount_residual'] = remaining_credit_amount
		credit_vals['amount_residual_currency'] = remaining_credit_amount_curr
		credit_vals['amount_residual_ref'] = remaining_credit_amount_curr_ref
	...
	def _prepare_exchange_difference_move_vals(self, amounts_list, company=None, exchange_date=None, **kwargs):
		...
		payment_names = ', '.join(filter(bool, self.mapped('payment_id.name')))
		base_label = _('Currency exchange rate difference')
		line_label = f"{base_label} - {payment_names}" if payment_names else base_label
