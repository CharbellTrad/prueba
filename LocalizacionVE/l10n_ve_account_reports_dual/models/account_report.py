...
class AccountReport(models.Model):
    ...
    # Fix: Check if name is string AND contains currency symbol 
    def _format_lines_for_display(self, lines, options):
        if options.get('selected_currency', 0) == self.env.company.currency_ref_id.id:
            currency = self.env.company.currency_ref_id
            for line in lines:
                for column in line['columns']:
                    ...
                    # Fix: Check if name is string AND contains currency symbol
                    if name and isinstance(name, str) and isinstance(no_format, (int, float)) and self.env.company.currency_id.symbol in name:
                        column['name'] = self.format_value(no_format, currency=currency, figure_type='monetary')
        return lines
