...
class AccountWithholdingIslrPaymentWizard(models.TransientModel):
    _name = 'account.withholding.islr.payment.wizard'
    _description = 'Wizard to pay multiple ISLR withholdings'

    withholding_ids = fields.Many2many(
        'account.withholding.islr',
        relation='acc_wh_islr_pay_wiz_rel',
        column1='wizard_id',
    ...
    def default_get(self, fields_list):
        res = super(AccountWithholdingIslrPaymentWizard, self).default_get(fields_list)
    ...
    def action_pay(self):
        ...
        # Se deja en estado borrador (draft)
        self.withholding_ids.write({'payment_id': payment.id})
        
        return {
            'name': _('Pago de Retenciones ISLR'),
